				...
				<div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-7">
                            ...
                            @endphp
                            @if(!empty(json_decode($product->size_heading)))
                            @php
                                $size_heading = implode(', ', json_decode($product->size_heading))
                            @endphp
                            @endif
                            <input type="text" class="form-control" name="size_heading[]" id="size_heading" value="{{ $size_heading }}" placeholder="{{ translate('Type to add size heading') }}" data-role="tagsinput"> -->
                        	@php
						        $psize_Array = json_decode($product->size_heading);
						        $selected_psizes = array();
							@endphp
							@foreach($psize_Array as $psize_name)
							    @php 
                        	...
                        	<select class="form-control demo-select2-size" name="size_heading[]" id="size_heading_id" multiple>
								@foreach (\App\Psize::orderBy('name', 'asc')->get() as $key => $psize)
									<option value='{{ $psize->name }}' <?php if(in_array($psize->name, $selected_psizes)) echo 'selected'?> >{{ $psize->name }}</option>
								@endforeach
							</select>
                        </div>
                    </div>
						...
						<div class="col-lg-7">
							...
							<select class="form-control demo-select2-size" name="size_id[]" id="size_id" multiple>
								@foreach (\App\Size::orderBy('name', 'asc')->get() as $key => $size)
									<option value="{{ $size->id }}" <?php if(in_array($size->id, $size_id)) echo 'selected'?> >{{ $size->name }}</option>
								@endforeach
							</select>
						</div>
					</div>
					    ...
					    @if(!$size_Array == null)
					        ...
					        @foreach($size_Array as $key => $option)
							    ...
							    <div class="form-group" data="{{$name}}">
							       <label class="col-lg-2 control-label">{{$size_name[0]}} (Inches)</label>
							       @if(!$option->values == null)
							        @foreach($option->values as $key => $val)
        					        <div class="col-lg-1">
        					            <input type="number" class="form-control" name="{{$name}}[]" id="" value="{{$val}}" placeholder="S">
        					        </div>
        					        @endforeach
        					        @endif
        					   </div>
					...
					<div class="customer_choice_options" id="customer_choice_options">
						@foreach (json_decode($product->choice_options) as $key => $choice_option)
							<div class="form-group">
								...
								@endforeach
	                        	<div class="col-lg-7">
		                        	<select class="form-control demo-select2-size" name="choice_options_{{ $choice_option->attribute_id }}[]" id="size_heading_id" multiple onchange="update_sku()">
										@foreach (\App\Psize::orderBy('name', 'asc')->get() as $key => $psize)
											<option value='{{ $psize->name }}' <?php if(in_array($psize->name, $selected_sizes)) echo 'selected'?> >{{ $psize->name }}</option>
										@endforeach
									</select>
								</div>
							</div>
			...
			<div class="panel">
				...
				<div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-9">
                            <textarea name="designdetails" rows="3" class="form-control editor">{{ $product->designdetails }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-2 control-label">{{translate('Size & Fit')}}</label>
                        <div class="col-lg-9">
                            <textarea name="sizefits" rows="3" class="form-control editor">{{ $product->sizefits }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-2 control-label">{{translate('Fabric & Care')}}</label>
                        <div class="col-lg-9">
                            <textarea name="fabriccare" rows="3" class="form-control editor">{{ $product->fabriccare }}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-lg-9">
                            <ul class="specification-list">
                                <!--<li><input type="text" class="form-control" name="speciftype" placeholder="Type" value="{{ $product->speciftype }}" /></li>
                                <li><input type="text" class="form-control" name="specifpattern" placeholder="Pattern" value="{{ $product->specifpattern }}" /></li>
                                <li><input type="text" class="form-control" name="specifoccasion" placeholder="Occasion" value="{{ $product->specifoccasion }}" /></li>
                                <li><input type="text" class="form-control" name="specifborder" placeholder="Border" value="{{ $product->specifborder }}" /></li>
                                <li><input type="text" class="form-control" name="specifsareefabric" placeholder="Saree Fabric" value="{{ $product->specifsareefabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifblouse" placeholder="Blouse" value="{{ $product->specifblouse }}" /></li>
                                <li><input type="text" class="form-control" name="specifblousefabric" placeholder="Blouse Fabric" value="{{ $product->specifblousefabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifwash" placeholder="Wash" value="{{ $product->specifwash }}" /></li>-->
                                
                                <li><input type="text" class="form-control" name="specifproducttype" placeholder="Product type" value="{{ $product->specifproducttype }}" /></li>
                                <li><input type="text" class="form-control" name="speciffabrictype" placeholder="Fabric type" value="{{ $product->speciffabrictype }}" /></li>
                                <li><input type="text" class="form-control" name="specifweave" placeholder="Weave" value="{{ $product->specifweave }}" /></li>
                                <li><input type="text" class="form-control" name="specifborder" placeholder="Border" value="{{ $product->specifborder }}" /></li>
                                <li><input type="text" class="form-control" name="specifoccasion" placeholder="Occasion" value="{{ $product->specifoccasion }}" /></li>
                                <li><input type="text" class="form-control" name="specifwash" placeholder="Wash" value="{{ $product->specifwash }}" /></li> 
                                <li><input type="text" class="form-control" name="speciffabric" placeholder="Fabric" value="{{ $product->speciffabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifblousefabric" placeholder="Blouse Fabric" value="{{ $product->specifblousefabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifdupattafabric" placeholder="Dupatta fabric" value="{{ $product->specifdupattafabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifbottomfabric" placeholder="Bottom fabric" value="{{ $product->specifbottomfabric }}" /></li>
                                <li><input type="text" class="form-control" name="specifsleevelength" placeholder="Sleeve length" value="{{ $product->specifsleevelength }}" /></li>
                                <li><input type="text" class="form-control" name="specifneck" placeholder="Neck" value="{{ $product->specifneck }}" /></li>
                                <li><input type="text" class="form-control" name="specifbottomshape" placeholder="Bottom Shape" value="{{ $product->specifbottomshape }}" /></li>
                                <li><input type="text" class="form-control" name="specifstylecode" placeholder="Style Code" value="{{ $product->specifstylecode }}" /></li>
                            </ul>
