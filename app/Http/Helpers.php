...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        if (OtpConfiguration::where('type', 'custom')->first()->value == 1) {
            //echo "sadasdasd";
            $url = 'http://bulksms.ind.in/api/swsend.asp?username=stitchdeal&password=68253113&sender=StitDL&sendto='.$to.'&message='.rawurlencode($text);
            //echo $url;
            $ch = curl_init();  
...
if (! function_exists('format_price')) {
    function format_price($price)
    {
        if(BusinessSetting::where('type', 'symbol_format')->first()->value == 1){
            //$discounted_price = str_replace(',', '' , home_discounted_price($detailedProduct->id));
            //$home_base_price = str_replace(',', '' , home_base_price($detailedProduct->id));
...
if (! function_exists('backend_discounted_base_price')) {
    function backend_discounted_base_price($id)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        $flash_deals = \App\FlashDeal::where('status', 1)->get();
