...
class CheckoutController extends Controller
{
    ...
    {
        //var_dump($request->payment_option);exit;
    ...
    public function checkout_done($order_id, $payment)
    {
        ...
        if (\App\Addon::where('unique_identifier', 'otp_system')->first() != null && \App\Addon::where('unique_identifier', 'otp_system')->first()->activated && \App\OtpConfiguration::where('type', 'otp_for_order')->first()->value == 1){
            
            try {
                $name = json_decode($order->shipping_address)->name;
                $order->phone = json_decode($order->shipping_address)->phone;
                $order->message = 'Hi '.$name.' Your Order '.$order->id.' of Rs '.$order->grand_total.'.00 has been confirmed. You will recieve an update on shipment. Call +91 88580 25450 for assistance.';
                $otpController = new OTPVerificationController;
                $otpController->send_code($order);
            } catch (\Exception $e) {
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if(env('DELHIVERY_API_MODE') == "ON"){
            ...
            $delhiveryExpressAPIController->pincodeServiceability($data);
            if (Session::has('pincode_servicable') && Session::get('pincode_servicable') == 0) {
                flash(translate("Unfortunately we do not ship to your pincode."))->warning();
                return back();
            }
        }
    ...
    public function order_confirmed(){
        ...
        if(env('DELHIVERY_API_MODE') == "ON"){
            $delhiveryFlowArr = array();
            $delhiveryExpressAPIController = new DelhiveryAPIController;
            $order = Order::findOrFail(Session::get('order_id'));
            /* new order creation */
            $responseArr = $delhiveryExpressAPIController->createOrder($order);
            //dd($responseArr);
            if(isset($responseArr['waybill'])){
                $delhiveryFlowArr['actions'][] = "Generated Waybill Number - ".$responseArr['waybill'];
                $delhiveryFlowArr['actions'][] = "Forward Order Created Successfully by Waybill Number - ".$responseArr['waybill'];
                $delhiveryFlowArr['order_create_response'] = $responseArr['response'];
            }
        }
        $order = Order::findOrFail(Session::get('order_id'));
        return view('frontend.order_confirmed', compact('order'));
    ...
    public function check_pincode_delhivery(Request $request)
    {
        $data['postal_code'] = $request->postal_code;
        if(env('DELHIVERY_API_MODE') == "ON"){
            $delhiveryExpressAPIController = new DelhiveryAPIController;
            ...
            if (Session::has('pincode_servicable') && Session::get('pincode_servicable') == 0) {
                ...
                flash(translate('Shipping delivery available.'))->success();
            }
        }
        return back();
    }
