...
class HomeController extends Controller
{
    ...
    public function load_scroll_sliders(Request $request){
        $product = Product::where('id',$request->product_id)->first();
        return view('frontend.partials.load_scroll_sliders',compact('product'));
    }
    ...
    public function search(Request $request)
    {
        $query = $request->q;
        $brand_id = "";
        $product_exit_brand_ids = array();
        if($request->has('brand')){
            $brand_id = (Brand::whereIn('slug', $request->brand)->first() != null) ? Brand::whereIn('slug', $request->brand)->first()->id : null;
        ...
        if($sort_by != null){
            switch ($sort_by) {
                ...
                case '4':
                    ...
                    break;
                case '5':
                    $best_seller_product_ids  = DB::table('order_details')->distinct('product_id')->pluck('product_id','product_id')->toArray();
                    $products = $products->whereIn('id',$best_seller_product_ids);
                    break;    
                default:
                    // code...
                    break;
        ...
        foreach ($non_paginate_products as $key => $product) {
            $product_exit_brand_ids[$product->brand_id] = $product->brand_id;
        }
        
        if($min_price != null && $max_price != null){
            $products = $products->where('discount_price', '>=', $min_price)->where('discount_price', '<=', $max_price);
        }
        ...
        /*$brands = array();
        if($request->has('brand')){
            $by_brand = Product::select('name','brand_id')->whereIn('brand_id', $brand_ids)->get();
            foreach ($by_brand as $key => $brand){
                $brands[] = $brand->name;
            }
            $products = $products->whereIn('name', $brands);
        }*/
        ...
        if($request->has('color')){
            foreach ($request->color as $key => $color) {
                if(!in_array($color, $selected_color)){
                    $selected_color[] = $color;
                    //$products = $products->where('colors', 'like', '%'.$color.'%');
                }
            }
            $products = $products->where(function ($query) use($selected_color) {
             for ($i = 0; $i < count($selected_color); $i++){
                $query->orwhere('colors', 'like',  '%' . $selected_color[$i] .'%');
             }      
            });
        }
        
        $products = filter_products($products)->paginate(30)->appends(request()->query());

        return view('frontend.product_listing', compact('request','products', 'query', 'category_id', 'subcategory_id', 'subsubcategory_id', 'brand_id', 'brand_ids', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color','product_exit_brand_ids'));
    }
    ...
    public function savecontacts(Request $request){
        ...
        $array['subject'] = "Contact enquiry";
        $array['from'] = env('MAIL_USERNAME');
        $array['content'] = $request->contactMessage;
        $array['sender'] = env('MAIL_USERNAME');
        $array['customerName'] = $request->customerName;
        $array['customerEmail'] = $email;
        $array['contactSubject'] = $request->contactSubject;
        $array['details'] = "Email Second";
        try {
            Mail::to(env('MAIL_FROM_ADDRESS'))->bcc(array($email))->queue(new ContactMailManager($array));
            flash(translate('Thank you for your enquiry.'))->success();
            return back();
        } catch (\Exception $e) {
